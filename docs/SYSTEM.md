...
## 8. 進行中と未決(2026年9月22日 12:00 JST 時点)
...
| | 状態 | 次に要るもの |
|---|---|---|
| **優先手数料の入札** | 2026年9月22日 11:30 JST に投入。**今まで一度も積んでいなかった**(定価で並んでいた) | `[実行] … 優先+Xgwei($0.XXXX)` が出るか確認 → **負けの増え方が鈍るか**。基準は下記。効かなければ `PRIORITY_FEE_SHARE=0` で即停止(再デプロイ不要) |
