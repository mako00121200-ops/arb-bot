...
指値の上限を 0.98 / 0.985 / 0.99 の3通り同時に試し、どれが一番儲かるかを実データで決める(9/26: 0.96/0.97 は10時間で約定0だったので上に寄せた)。
...
結果は画面の「両側買い」カード、ログの `[両側]` 行、JSONL の `pair` 行。累計は `pair.json`。
**2026年9月26日に停止(既定 `PAIR=off`)**: 26市場 −$11.80、修正後も組がそろった市場0。`PAIR=paper` で再開できる。

## 測り方の点検(30分ごと、2026年9月26日〜)

「4時間測ってから測り方の間違いに気づく」を防ぐため、collector が30分ごとに `[点検 30分]` を1行出し、画面の最上段にも表示する。

| 項目 | 何を疑うか |
|---|---|
| チェーン約定の件数・到着遅れ | 0件なら読み取り停止。到着遅れが150秒を超えると、損益確定(決済の180秒後)に間に合わない約定が出る |
| 終盤: 値段一致 / 時間外 / 数えた | 値段は合うのに「指値の有効時間外」ばかりなら、有効時間(置いた時刻+遅延)の決め方を疑う |
| 遅着 | 損益確定の後に届いた約定。1件でも出たら待ち時間が短い |
| 両側: そろった / 片側 | 片側の方が多ければ、指値が古くて拾われている(逆選択)を疑う |
| 板の古さ・Binance の停止 | 入力データが止まっていないか |

警告があれば `警告:` の後に並ぶ。`AUDIT_INTERVAL_MS` で間隔を変えられる(既定30分)。
