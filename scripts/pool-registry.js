...
// ④除外:   実測手数料が1%を超えるプール(税トークン)、異常なリターンの
//          案件(ハニーポット)は恒久的に除外する。
// ⑤記録:   検出から結果までを1件ずつ記録簿に残し、後から集計できるようにする。
//
// [大口取引の優先]
// 本物の裁定機会は「常にある価格差」ではなく、大きな取引の直後に生まれる。
// 準備量が大きく動いたプールは、そうでないプールより優先して扱う。
...
const stats = {
  ...
  skippedCooldown: 0, trapsRejected: 0, taxTokensRejected: 0, bigMoves: 0,
...
function heartbeat() {
  ...
  console.log(`[生存] スキャン${stats.scans} 精査${stats.examined} 黒字${stats.profitableFound} 実行${stats.executed}/${stats.failed} 大口${stats.bigMoves} 罠${stats.trapsRejected} 税${stats.taxTokensRejected} 無効${stats.disabled} Sync一致${stats.syncMatched}/未知${stats.syncUnknown} 取込${stats.adopted} 手数料${stats.feeProbed} 行列[${queued || "空"}]`);
}
...
function renderPage() {
  ...
  const missedRows = sum.topMissed.map((m, i) => `<tr><td>${i+1}</td>
    <td style="font-size:9px">${m.kind} ${m.chain}<br>${m.label}</td>
    <td style="font-size:9px">${OUTCOME_LABEL[m.outcome] || m.outcome}</td>
    <td style="text-align:right">$${(m.tradeAmountUsd||0).toFixed(2)}</td>
    <td style="text-align:right;color:#e8a33d">+$${(m.netProfitUsd||0).toFixed(4)}</td></tr>`).join('') || `<tr><td colspan="5" style="color:#888">記録がありません</td></tr>`;
...
<table><thead><tr><th>結果</th><th style="text-align:right">件数</th></tr></thead><tbody>${outcomeRows}</tbody></table>
<div class="note"><strong>取り逃した黒字の上位</strong>(なぜ取れなかったかが分かります)</div>
<table><thead><tr><th>#</th><th>経路</th><th>理由</th><th style="text-align:right">投入</th><th style="text-align:right">見込み</th></tr></thead><tbody>${missedRows}</tbody></table></div>
...
<div class="note">精査した経路${stats.examined.toLocaleString()}件。実測手数料が${TAX_TOKEN_FEE_BPS}bps(1%)を超えるプールは「送金時に税を取るトークン」として恒久的に除外します。罠${stats.trapsRejected}件 / 無効化${stats.disabled}件</div></div>
...
<div><div class="v">${matchRate}%</div><div class="l">取引の捕捉率</div></div></div>
<div class="note">${syncLine}<br>実測ガス代(2step): ${gasLine}<br>待ち行列: ${Object.entries(rpc).map(([c, v]) => `${c}:${v.queued}`).join(' / ')}<br>
プール${s.totalPools.toLocaleString()} / 裁定候補${s.arbitragablePairs.toLocaleString()}ペア / 自動発見${stats.adopted.toLocaleString()}件</div></div>
...
<div class="card"><h2>② 活発なプールの自動発見</h2><div class="note">Syncイベントが届いた未知のプールは、取引が起きている証拠なので自動的に取り込みます。準備量がどれだけ動いたかも記録し、大口取引の直後を見分けられるようにしています。</div></div>
<div class="card"><h2>③ 手数料の実測</h2><div class="note">Uniswap V2形式のプールには手数料を問い合わせる関数がありません。そこでガス見積もり(無料)の拒否理由を使って実測します。「K」は想定が低すぎる合図、「not profitable」は想定が正しい合図です。</div></div>
<div class="card"><h2>④ 税トークンの除外</h2><div class="note">正常なDEXの手数料は最大でも1%です。実測が1%を超えるのは「送金時に税を取るトークン」であり、構造上裁定できません。該当するプールは恒久的に除外します。</div></div>
<div class="card"><h2>⑤ 記録簿</h2><div class="note">検出から結果までを1件ずつ記録し、「黒字だったのになぜ取れなかったか」を後から集計できるようにしています。</div></div>
